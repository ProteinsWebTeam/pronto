...
$(function () {
    utils.getJSON('/api/database/', (data, status) => {
        const html = [];
        data.results.forEach(db => {
            html.push(
                [
                    ...
                    '<td><a href="/database/'+ db.shortName +'">'+ db.count.toLocaleString() +'</a></td>',
                    '<td>'+ db.countIntegrated.toLocaleString() +'</td>',
                    '<td><a href="/database/'+ db.shortName +'?unintegrated=exist">'+ db.countUnintegrated.toLocaleString() +'</a></td>',
                    '</tr>'
                ].join('')
            );
        });

        document.querySelector('#databases tbody').innerHTML = html.join('');
    });
